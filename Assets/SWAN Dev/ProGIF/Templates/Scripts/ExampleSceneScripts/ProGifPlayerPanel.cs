...
public class ProGifPlayerPanel : MonoBehaviour
{
	...
	public void Play()
	{
		...
		{
			case DataPathTypeDebug.UrlInputField:
				break;
		...
		if (localDebugDataPath != DataPathTypeDebug.UrlInputField && !gifPath.ToLower().Contains(".gif")) gifPath += ".gif";
	...
	/// Find and select the path type (for testing in Unity Editor only, may require more handling for some platforms)
	/// </summary>
	private DataPathTypeDebug _CheckAndSelectPathType(ref string gifPath)
    {
		...
		if (pathLower.StartsWith("assets/streamingassets/"))
		{
			...
			return DataPathTypeDebug.StreamingAssetsPath;
		}
		else if (pathLower.StartsWith("assets/"))
		{
			gifPath = gifPath.Substring("assets/".Length);
			return DataPathTypeDebug.DataPath;
		}
		else if (pathLower.StartsWith("http"))
		{
			return DataPathTypeDebug.UrlInputField;
		}
		else
		{
			return DataPathTypeDebug.PersistentDataPath;
		}
	}
