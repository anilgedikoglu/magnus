...
public class cgChessBoardScript : MonoBehaviour
{
    ...
    DateTime _engineOperationStartTime = DateTime.Now;
    public float _secondMessageDelay;
    bool check;
    public RectTransform progressBarRt;
    public Transform progressBar0Pivot;
    bool reseted;
    bool isCastling;
    bool deadPieces;
    [HideInInspector] public bool isGameEnd;
    [HideInInspector] public float currentEngineProgress;
    ...
    BoardMode lastBoardMode = BoardMode.PlayerVsEngine; 
    ...
    public void start(cgBoard customBoard = null, BoardMode mode = BoardMode.Undefined)
    {
        ...
        {
            gameObject.transform.parent.transform.eulerAngles = new Vector3(gameObject.transform.parent.transform.eulerAngles.x, gameObject.transform.parent.transform.eulerAngles.y, 180);
            
            foreach (cgChessPieceScript element in _livePieces)
            {
                element.gameObject.transform.eulerAngles = new Vector3(element.gameObject.transform.eulerAngles.x, element.gameObject.transform.eulerAngles.y, 0);
            }
        }
        else if (mode == BoardMode.PlayerVsEngine)
        {
            gameObject.transform.parent.transform.eulerAngles = new Vector3(gameObject.transform.parent.transform.eulerAngles.x, gameObject.transform.parent.transform.eulerAngles.y, 0);
        ...
        if(lastBoardMode != Mode)
        {
            UnityEngine.Debug.Log("satranc tahtasi cevrildi");
            FlipBoard();
            lastBoardMode = Mode;
        }
        flipTarget.transform.position = new Vector3(flipTarget.transform.position.x, transform.parent.position.y - 5.5f, flipTarget.transform.position.z);

        float xScale = (canvas.rect.width / 540f);
        float yScale = (canvas.rect.height / 1080f);

        if (xScale < yScale)
        {
            transform.parent.localScale = new Vector3(xScale, xScale, (transform.parent.localScale.z));
        }
        else
        {
            transform.parent.localScale = new Vector3(yScale, yScale, (transform.parent.localScale.z));
        }
        progressBarRt.parent.gameObject.GetComponent<RectTransform>().localScale = new Vector3(transform.parent.localScale.x, progressBarRt.localScale.y, progressBarRt.localScale.z);
    }
    /// <summary>
    ...
    void Update()
    {
        ...
        lock (_engineProgress)
        {
            if (_engineProgress.Count > 0 && engineProgressBar != null)
            {
                ...
                currentEngineProgress = _engineProgress[0];
                ...
                progressBarRt.localScale = new Vector3(progress, progressBarRt.localScale.y, progressBarRt.localScale.z);
            }
        }
        ...
        if (Mode == BoardMode.EngineVsPlayer)
        {
            progressBarRt.parent.gameObject.GetComponent<RectTransform>().position = new Vector3(progressBarRt.parent.gameObject.GetComponent<RectTransform>().position.x, Camera.main.WorldToScreenPoint(progressBar0Pivot.position).y, progressBarRt.parent.gameObject.GetComponent<RectTransform>().position.z);
    ...
    public ChessSettings chessSettings;
    ...
    public void SohbetiAyarla(string mod)
    {
        if (!isGameEnd)
        {
    ...
    public void HareketiDegerlendir()
    {
        float chance = UnityEngine.Random.Range(0f, 100f);

        if (chessSettings.hareketDegerlendirmeSansi > chance && !check && !isCastling && !deadPieces)
        {
            SohbetiAyarla(chessSettings.hareketDegerlendirmeModu);
        }
    }

    public void KendiHareketiniDegerlendir()
    {
        float chance = UnityEngine.Random.Range(0f, 100f);

        if (chessSettings.kendiHareketiniDegerlendirmeSansi > chance && !check && !isCastling && !deadPieces)
        {
            SohbetiAyarla(chessSettings.kendiHareketiniDegerlendirmeModu);
        }
    }

    public void CastlingDegerlendir()
    {
        float chance = UnityEngine.Random.Range(0f, 100f);

        if (chessSettings.hareketDegerlendirmeSansi >= 0)
        {
            SohbetiAyarla(chessSettings.castlingModu);
        }
    }
    ...
    private void _pieceUp(cgChessPieceScript piece)
    {
        if (_downPiece != null)
        {
            if (playerCanMove || Mode == BoardMode.PlayerVsPlayer)
            {
                ...
                {
                    if (piece.square != closestSquare)
                    {
                        SohbetiAyarla(chessSettings.tasOynanamazModu);
                    }
    ...
    private void _gameOver(bool whiteWins, bool blackWins)
    {
        ...
        if (whiteWins)
        {
            ...
            if (Mode == BoardMode.PlayerVsEngine)
            {
                SohbetiAyarla(chessSettings.kazanmaModu);
                Instantiate(chatManager.fireworkPrefab, chatManager.fireworkPivotTr);
                isGameEnd = true;
            }
            else if (Mode == BoardMode.EngineVsPlayer)
            {
                SohbetiAyarla(chessSettings.kaybetmeModu);
                isGameEnd = true;
            }
        ...
        else if (!blackWins && !whiteWins)
        {
            ...
            SohbetiAyarla(chessSettings.berabereModu);
            isGameEnd = true;
        }
        ...
        UnityEngine.Debug.Log(gameOverString);
    }

    public void SetActive(bool value)
    {
        FindObjectOfType<GyroCamera>().StartChessBoard();

        progressBarRt.parent.gameObject.SetActive(value);

        if (value && !reseted)
        {
            UnityEngine.Debug.Log("board reseted");
            ResetBoard();
            reseted = true;
        }
    ...
    /// </summary>
    private void _checkGameOver(bool isEngine)
    {
        ...
        else if (isChecked)
        {
            ...
            if (!isEngine && !check)
            {
                SohbetiAyarla(chessSettings.checkKullaniciModu);
                check = true;
            }
            else
            {
                SohbetiAyarla(chessSettings.checkMagnusModu);
                check = true;
            }
    ...
    private void _makeMove(cgSimpleMove move)
    {
        ...
        piece.SetType(_abstractBoard.squares[move.to]);

        whiteTurnToMove = _abstractBoard.whiteTurnToMove;

        if (Mode == cgChessBoardScript.BoardMode.PlayerVsEngine)
            _checkGameOver(_abstractBoard.whiteTurnToMove);
        else
            _checkGameOver(!_abstractBoard.whiteTurnToMove);
    ...
    private void _makeEngineMoveMulti(cgBoard board, bool moveAsWhiteP, Action<List<cgSimpleMove>> callback) {
        _secondMessageDelay = UnityEngine.Random.Range(2f, 8f);
        _engine._minMoveDelay = UnityEngine.Random.Range(1f, 3f);

        _engineOperationStartTime = DateTime.Now;
        _engine._engineOperationStartTime = DateTime.Now;
             ...
             DoWorkEventArgs e) =>
        {
            ...
            _engine.timeSinceStart = DateTime.Now.Subtract(_engineOperationStartTime);
    ...
    {
        TimeSpan timeSinceStart = DateTime.Now.Subtract(_engineOperationStartTime);

        if (_secondMessageDelay - timeSinceStart.Seconds < 0)
        {
            UnityEngine.Debug.Log("Worker thread done and done. ");
            KendiHareketiniDegerlendir();
        }
        else
        {
            StartCoroutine(KendiHareketiniDegerlendirIEnumerator((int)_secondMessageDelay - timeSinceStart.Seconds));
        }
    }

    IEnumerator KendiHareketiniDegerlendirIEnumerator(int time)
    {
    ...
    private void _setDeadPiece(cgChessPieceScript cp)
    {
        ...
        if (!_deadPieces.Contains(cp))
        {
            ...
            if (cp.white)
            {
                ...
                if (Mode == cgChessBoardScript.BoardMode.PlayerVsEngine)
                {
                    MagnusTasYediModuAyarla(cp.type);
                }
                else if (Mode == cgChessBoardScript.BoardMode.EngineVsPlayer)
                {
                    KullaniciTasYediModuAyarla(cp.type);
                }
            }
            else if (!cp.white)
            {
    ...
    void KullaniciTasYediModuAyarla(cgChessPieceScript.Type type)
    {
        deadPieces = true;
        switch (type)
        {
            case cgChessPieceScript.Type.BlackPawn:
                SohbetiAyarla(chessSettings.kullaniciPiyonYediModu);
                break;
            case cgChessPieceScript.Type.WhitePawn:
                SohbetiAyarla(chessSettings.kullaniciPiyonYediModu);
                break;
            case cgChessPieceScript.Type.BlackRook:
                SohbetiAyarla(chessSettings.kullaniciKaleYediModu);
                break;
            case cgChessPieceScript.Type.WhiteRook:
                SohbetiAyarla(chessSettings.kullaniciKaleYediModu);
                break;
            case cgChessPieceScript.Type.BlackKnight:
                SohbetiAyarla(chessSettings.kullaniciAtYediModu);
                break;
            case cgChessPieceScript.Type.WhiteKnight:
                SohbetiAyarla(chessSettings.kullaniciAtYediModu);
                break;
            case cgChessPieceScript.Type.BlackBishop:
                SohbetiAyarla(chessSettings.kullaniciFilYediModu);
                break;
            case cgChessPieceScript.Type.WhiteBishop:
                SohbetiAyarla(chessSettings.kullaniciFilYediModu);
                break;
            case cgChessPieceScript.Type.BlackKing:
                SohbetiAyarla(chessSettings.kullaniciSahYediModu);
                break;
            case cgChessPieceScript.Type.WhiteKing:
                SohbetiAyarla(chessSettings.kullaniciSahYediModu);
                break;
            case cgChessPieceScript.Type.BlackQueen:
                SohbetiAyarla(chessSettings.kullaniciVezirYediModu);
                break;
    ...
    void MagnusTasYediModuAyarla(cgChessPieceScript.Type type)
    {
        deadPieces = true;
        switch (type)
        {
            case cgChessPieceScript.Type.BlackPawn:
                SohbetiAyarla(chessSettings.magnusPiyonYediModu);
                break;
            case cgChessPieceScript.Type.WhitePawn:
                SohbetiAyarla(chessSettings.magnusPiyonYediModu);
                break;
            case cgChessPieceScript.Type.BlackRook:
                SohbetiAyarla(chessSettings.magnusKaleYediModu);
                break;
            case cgChessPieceScript.Type.WhiteRook:
                SohbetiAyarla(chessSettings.magnusKaleYediModu);
                break;
            case cgChessPieceScript.Type.BlackKnight:
                SohbetiAyarla(chessSettings.magnusAtYediModu);
                break;
            case cgChessPieceScript.Type.WhiteKnight:
                SohbetiAyarla(chessSettings.magnusAtYediModu);
                break;
            case cgChessPieceScript.Type.BlackBishop:
                SohbetiAyarla(chessSettings.magnusFilYediModu);
                break;
            case cgChessPieceScript.Type.WhiteBishop:
                SohbetiAyarla(chessSettings.magnusFilYediModu);
                break;
            case cgChessPieceScript.Type.BlackKing:
                SohbetiAyarla(chessSettings.magnusSahYediModu);
                break;
            case cgChessPieceScript.Type.WhiteKing:
                SohbetiAyarla(chessSettings.magnusSahYediModu);
                break;
            case cgChessPieceScript.Type.BlackQueen:
                SohbetiAyarla(chessSettings.magnusVezirYediModu);
                break;
            case cgChessPieceScript.Type.WhiteQueen:
                SohbetiAyarla(chessSettings.magnusVezirYediModu);
                break;
