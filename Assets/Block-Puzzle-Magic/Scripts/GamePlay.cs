...
public class GamePlay : Singleton<GamePlay>, IPointerDownHandler,IPointerUpHandler, IBeginDragHandler,IDragHandler
{
    ...
    void Start ()
	{
		...
		//Oyun basladığı anda bu değişkeni daha önceden kalan bir değer görünmesin diye sıfırlıyoruz.
		chatManager.PlayerDataManager.AddElementToChatVariableList(chatManager.magnuTrisSettings.oyunSonuMesajiDegiskenAdi, "", false);
    ...
    private void Update()
    {
        if (!chatManager.magnuTrisSettings.IsMagnuTrisMod(chatManager.PlayerDataManager.GetChatVariableValue("mod")))
        {
    ...
    public void OnBeginDrag (PointerEventData eventData)
	{
		if (currentShape != null) {
			Vector3 pos = gameCamera.ScreenToWorldPoint (eventData.position);
		...
		if (eventData.pointerCurrentRaycast.gameObject != null) {
			...
			if (clickedObject.GetComponent<ShapeInfo>() != null) 
			{
				if (clickedObject.transform.childCount > 0) {
					...
					Vector3 pos = gameCamera.ScreenToWorldPoint (eventData.position);
					currentShape.transform.localScale = new Vector3(1.2f, 1.2f, 1.2f) * GameBoardGenerator.Instance.BlockSize;
	...
	public void OnPointerUp (PointerEventData eventData)
	{
		if (currentShape != null) {
			...
			if (highlightingBlocks.Count > 0) 
			{
				...
				currentShape.transform.DOScale (Vector3.one * 0.75F, 0.5F);
				#endif
				currentShape = null;
	...
	public void OnDrag (PointerEventData eventData)
	{
		if (currentShape != null) {
			...
			currentShape.transform.localPosition = new Vector3(currentShape.transform.localPosition.x, currentShape.transform.localPosition.y, transform.localPosition.z);
	...
	void SetImageToPlacingBlocks ()
	{
		if (highlightingBlocks != null && highlightingBlocks.Count > 0) 
		{
			...
			if (GameController.gameMode != GameMode.ADVANCE || GameController.gameMode != GameMode.CHALLENGE)
				chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.sekiller[currentShape.ShapeID]);
		}
	...
	IEnumerator BreakAllCompletedLines(List<List<Block>> breakingRows, List<List<Block>> breakingColumns, int placingShapeBlockCount)
	{
		...
		if (TotalBreakingLines == 1) {
			...
			if (!isGameEnded)
				chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.yokEdilenSatir1);
			...
			if (!isGameEnded)
				chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.yokEdilenSatir2);
			...
			if (!isGameEnded)
				chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.yokEdilenSatir3);
			...
			if (TotalBreakingLines == 4) {
				...
				if (!isGameEnded)
					chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.yokEdilenSatir4);
			} else {
				ScoreManager.Instance.AddScore ((300 * TotalBreakingLines)+ (placingShapeBlockCount * 10));
				if (!isGameEnded)
					chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.yokEdilenSatirFazla);
			}
		}
	...
	public void OnGameOver()
	{
        switch (GamePlayUI.Instance.currentGameOverReson)
        {
			case GameOverReason.BOMB_COUNTER_ZERO:
				chatManager.PlayerDataManager.AddElementToChatVariableList(chatManager.magnuTrisSettings.oyunSonuMesajiDegiskenAdi, chatManager.magnuTrisSettings.oyunSonuMesajiBombaPatladi, false);
				break;
			case GameOverReason.OUT_OF_MOVES:
				chatManager.PlayerDataManager.AddElementToChatVariableList(chatManager.magnuTrisSettings.oyunSonuMesajiDegiskenAdi, chatManager.magnuTrisSettings.oyunSonuMesajiHamleKalmadi, false);
				break;
			case GameOverReason.TIME_OVER:
				chatManager.PlayerDataManager.AddElementToChatVariableList(chatManager.magnuTrisSettings.oyunSonuMesajiDegiskenAdi, chatManager.magnuTrisSettings.oyunSonuMesajiSureDoldu, false);
				break;

		}

		//Bu deger aşağıdaki kodlardan önce kontrol edilir. Nedeni belli :))
		chatManager.PlayerDataManager.AddElementToChatVariableList("magnutris eski en yuksek skor", PlayerPrefs.GetInt("BestScore_" + GameController.gameMode.ToString(), 0).ToString());

		if(PlayerPrefs.GetInt("BestScore_" + GameController.gameMode.ToString(), 0) < ScoreManager.Instance.Score)
        {
			chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.enYuksekSkorGecildiModu);
        }
        else
        {
			chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.enYuksekSkorGecilmediModu);
		}

		int kazanma = chatManager.PlayerDataManager.GetChatVariableValueInt("magnutris kazanma");
		if (ScoreManager.Instance.Score > chatManager.magnusPreferences.magnuTris.baseScore + chatManager.magnusPreferences.magnuTris.increaseAmount * kazanma)
		{
			chatManager.PlayerDataManager.AddElementToChatVariableList("magnutris kazanma", (kazanma + 1).ToString());
		}

		GameObject gameOverScreen = StackManager.Instance.gameOverScreen;
		gameOverScreen.Activate();		
		//Asagisi max skoru ayarlar
		...
		chatManager.PlayerDataManager.AddElementToChatVariableList("magnutris son skor", ScoreManager.Instance.Score.ToString());
		chatManager.PlayerDataManager.AddElementToChatVariableList("magnutris en yuksek skor", FindBestScore().ToString());
	...
	int FindBestScore()
    {
		int classicModeBest = PlayerPrefs.GetInt("BestScore_" + GameMode.CLASSIC, ScoreManager.Instance.Score);
		int blastModeBest = PlayerPrefs.GetInt("BestScore_" + GameMode.CLASSIC, ScoreManager.Instance.Score);
		int timerModeBest = PlayerPrefs.GetInt("BestScore_" + GameMode.CLASSIC, ScoreManager.Instance.Score);
		int advancedModeBest = PlayerPrefs.GetInt("BestScore_" + GameMode.CLASSIC, ScoreManager.Instance.Score);

		List<int> degerler = new List<int> { classicModeBest, blastModeBest, timerModeBest, advancedModeBest };
		degerler.Sort();

		for(int i =0; i < degerler.Count; i++)
        {
			Debug.Log(degerler[i]);
        }

		return degerler[degerler.Count - 1];
	}
	...
	void PlaceBombAtRow(int rowIndex)
	{
		...
		if (!isGameEnded)
		{
			chatManager.ClickVirtualButton(chatManager.magnuTrisSettings.bombaYerlestirme);
		}
	}
